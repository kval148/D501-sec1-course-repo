...
def test_class_names(data):
    ...
    assert data['genre'].isin(known_classes).all(), "Column genre contains 1+ values not in known_classes"
...
def test_column_ranges(data):
    ...
    for col_name, (minimum, maximum) in ranges.items():
        ...
        assert data[col_name].dropna().between(minimum, maximum).all(), (
            f"Column {col_name} failed value range test. "
            f"Minimum should be {minimum} and maximum should be {maximum}, "
            f"but min={data[col_name].min()} and max={data[col_name].max()}."
        )
